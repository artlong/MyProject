...
public class TestUtility {
	 ...
	 * 
	 * @param code
	 *            A code (currently 0 = California, 1 = Nevada).
	 * @param name
	 *            The bean name to create.
	 ...
	 * 
	 * @return A <code>Map</code> of beans containing a <code>List</code> of all
	 *         <code>Divisions</code>.
	 */
	 ...
	 * 
	 * @param code
	 *            Determines which division, 0-7.
	 * @param name
	 *            This becomes the bean name of the <code>Division</code>.
	 * @return A <code>Map</code> of beans, containing a specific
	 ...
	 * 
	 * @return A <code>Map</code> of beans containing a <code>List</code> of 100
	 *         dummy <code>Divisions</code>.
	 * @since 0.8.0
	 */
	 ...
	 * 
	 * @param d
	 *            Determines which dummy "division".
	...
	public static Map<String, Object> getHyperlinkData() {
		...
		hyperlinks.add(new HyperlinkData(HyperlinkTag.TYPE_URL, "http://jett.sourceforge.net", "JETT on SourceForge"));
		hyperlinks.add(new HyperlinkData(HyperlinkTag.TYPE_EMAIL, "mailto:jett-users@lists.sourceforge.net", "Email jett-users"));
		hyperlinks.add(new HyperlinkData(HyperlinkTag.TYPE_FILE, "../templates/HyperlinkTagTemplate.xlsx", "Template For This Test (.xlsx)"));
		hyperlinks.add(new HyperlinkData(HyperlinkTag.TYPE_DOC, "'Target Sheet'!B3", "Intra-spreadsheet Link"));
	...
	public static String getStringCellValue(Sheet sheet, int row, int col) {
		...
		if (r != null) {
			...
			if (c != null) return c.getStringCellValue();
		}
		return null;
	}
	...
	public static boolean getBooleanCellValue(Sheet sheet, int row, int col) {
		...
		if (r != null) {
			Cell c = r.getCell(col);
			if (c != null) return c.getBooleanCellValue();
		}
		return false;
	}
	...
	public static RichTextString getRichTextStringCellValue(Sheet sheet, int row, int col) {
		...
		if (r != null) {
			Cell c = r.getCell(col);
			if (c != null) return c.getRichStringCellValue();
		}
		return null;
	}
	...
	public static double getNumericCellValue(Sheet sheet, int row, int col) {
		...
		if (r != null) {
			Cell c = r.getCell(col);
			if (c != null) return c.getNumericCellValue();
		}
		return Double.NaN;
	}
	...
	public static String getFormulaCellValue(Sheet sheet, int row, int col) {
		...
		if (r != null) {
			Cell c = r.getCell(col);
			if (c != null) {
				// The HSSF Formula Parser strips whitespaces outside of
				// strings, but the
	...
	public static Comment getComment(Sheet sheet, int row, int col) {
		...
		if (r != null) {
			Cell c = r.getCell(col);
			if (c != null) return c.getCellComment();
		}
		return null;
	}
	...
	public static boolean isMergedRegionPresent(Sheet sheet, CellRangeAddress region) {
		...
		for (int i = 0; i < numMergedRegions; i++) {
			...
			if (candidate.getFirstRow() == region.getFirstRow() && candidate.getLastRow() == region.getLastRow() && candidate.getFirstColumn() == region.getFirstColumn()
					&& candidate.getLastColumn() == region.getLastColumn()) { return true; }
		}
		return false;
	}
	...
	public static short getCellFillPattern(Sheet sheet, int row, int col) {
		...
		if (r != null) {
			Cell c = r.getCell(col);
			if (c != null) { return c.getCellStyle().getFillPattern(); }
		}
		return CellStyle.NO_FILL;
	}
	
	/**
	 * Returns the font color, as a hex string, on the given <code>Font</code>.
	 * 
	 * @param workbook
	 *            The <code>Workbook</code> on which the <code>Font</code> is
	 *            found.
	 * @param font
	 *            A <code>Font</code>.
		...
		if (r != null) {
			...
			if (c != null) {
				...
				if (cs instanceof HSSFCellStyle) {
					...
					throw new IllegalArgumentException("Unexpected type of CellStyle for cell on sheet " + sheet.getSheetName() + ", row " + row + ", col " + col);
				}
				return SheetUtil.getColorHexString(color);
			}
		}
	...
	public static Hyperlink getHyperlink(Sheet sheet, int row, int col) {
		...
		if (r != null) {
			Cell c = r.getCell(col);
			if (c != null) { return c.getHyperlink(); }
		}
		return null;
	}
	...
	public static Cell getCell(Sheet sheet, int row, int col) {
		Row r = sheet.getRow(row);
		if (r != null) return r.getCell(col);
		return null;
	}
