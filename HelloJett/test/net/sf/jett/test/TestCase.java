...
public abstract class TestCase {
	...
	public void testXls() throws IOException, InvalidFormatException {
		...
		if (!fOutputDir.exists() && !fOutputDir.mkdirs()) { throw new RuntimeException("Couldn't create output directory: " + OUTPUT_DIR); }
		String excelNameBase = getExcelNameBase();
		genericTest(TEMPLATES_DIR + excelNameBase + TEMPLATE_SUFFIX + XLS_EXT, OUTPUT_DIR + excelNameBase + OUTPUT_SUFFIX + XLS_EXT);
	}
	...
	public void testXlsx() throws IOException, InvalidFormatException {
		...
		if (!fOutputDir.exists() && !fOutputDir.mkdirs()) { throw new RuntimeException("Couldn't create output directory: " + OUTPUT_DIR); }
		String excelNameBase = getExcelNameBase();
		genericTest(TEMPLATES_DIR + excelNameBase + TEMPLATE_SUFFIX + XLSX_EXT, OUTPUT_DIR + excelNameBase + OUTPUT_SUFFIX + XLSX_EXT);
	}
	...
	public void testXlsFiles() throws IOException, InvalidFormatException {
		...
		if (!fOutputDir.exists() && !fOutputDir.mkdirs()) { throw new RuntimeException("Couldn't create output directory: " + OUTPUT_DIR); }
		String excelNameBase = getExcelNameBase();
		genericTestFiles(TEMPLATES_DIR + excelNameBase + TEMPLATE_SUFFIX + XLS_EXT, OUTPUT_DIR + excelNameBase + FILES_IND + OUTPUT_SUFFIX + XLS_EXT);
	}
	...
	public void testXlsxFiles() throws IOException, InvalidFormatException {
		...
		if (!fOutputDir.exists() && !fOutputDir.mkdirs()) { throw new RuntimeException("Couldn't create output directory: " + OUTPUT_DIR); }
		String excelNameBase = getExcelNameBase();
		genericTestFiles(TEMPLATES_DIR + excelNameBase + TEMPLATE_SUFFIX + XLSX_EXT, OUTPUT_DIR + excelNameBase + FILES_IND + OUTPUT_SUFFIX + XLSX_EXT);
	}
	 ...
	 *             If the input spreadsheet is invalid.
	 */
	protected void genericTest(String inFilename, String outFilename) throws IOException, InvalidFormatException {
		...
		try {
			...
			if (isMultipleBeans()) {
				...
				workbook = transformer.transform(fileIn, myTemplateSheetNames, myResultSheetNames, myListOfBeansMaps);
			} else {
				if (!amISetup) {
			...
			try {
				if (!(workbook instanceof HSSFWorkbook)) check(workbook);
	 ...
	 * @since 0.2.0
	 */
	protected void genericTestFiles(String inFilename, String outFilename) throws IOException, InvalidFormatException {
		InputStream fileIn = null;
		Workbook workbook;
		try {
			ExcelTransformer transformer = new ExcelTransformer();
			...
			if (isMultipleBeans()) {
				...
				assertNotNull(myResultSheetNames);
				assertNotNull(myListOfBeansMaps);
				transformer.transform(inFilename, outFilename, myTemplateSheetNames, myResultSheetNames, myListOfBeansMaps);
			} else {
				if (!amISetup) {
					myBeansMap = getBeansMap();
					amISetup = true;
