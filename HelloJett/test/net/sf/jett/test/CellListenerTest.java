...
public class CellListenerTest extends TestCase {
	...
	protected void check(Workbook workbook) {
		...
		for (int i = 0; i < 58; i++) {
			...
			if (population >= 1000000) assertEquals("Expected bold font at row " + (i + 2) + ", cell 1", Font.BOLDWEIGHT_BOLD, popFont.getBoldweight());
			else assertEquals("Expected not bold font at row " + (i + 2) + ", cell 1", Font.BOLDWEIGHT_NORMAL, popFont.getBoldweight());
			...
			if (area >= 10000) assertTrue("Expected italic font at row " + (i + 2) + ", cell 2", areaFont.getItalic());
			else assertFalse("Expected not italic font at row " + (i + 2) + ", cell 2", areaFont.getItalic());
		}
		for (int i = 0; i < 17; i++) {
			Row row = cellListener.getRow(i + 2);
			...
			if (population >= 1000000) assertEquals("Expected bold font at row " + (i + 2) + ", cell 7", Font.BOLDWEIGHT_BOLD, popFont.getBoldweight());
			else assertEquals("Expected not bold font at row " + (i + 2) + ", cell 7", Font.BOLDWEIGHT_NORMAL, popFont.getBoldweight());
			Cell nvArea = row.getCell(8);
			short areaFontIdx = nvArea.getCellStyle().getFontIndex();
			Font areaFont = workbook.getFontAt(areaFontIdx);
			double area = nvArea.getNumericCellValue();
			if (area >= 10000) assertTrue("Expected italic font at row " + (i + 2) + ", cell 8", areaFont.getItalic());
			else assertFalse("Expected not italic font at row " + (i + 2) + ", cell 8", areaFont.getItalic());
		}
		Sheet before = workbook.getSheetAt(1);
		assertEquals("California", TestUtility.getStringCellValue(before, 0, 1));
		assertEquals("The CellListener will replace the above content with ${california.name}", TestUtility.getStringCellValue(before, 1, 1));
	}
